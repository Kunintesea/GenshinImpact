...
void Enemy::update(float t)
{
	...
	else if (!leisure)
	{
		...
		if (m_name == "HellDog")
		{
			...
			//如果距离玩家超过600，在原地释放技能
			if (playerPosition.getDistance(this->getPosition()) > 600 && m_E_CD <= 0 && !isDead)
			{
			      //释放技能
			      skillE();
			}
...
void Enemy::skillQ()
{
      ...
      if (m_name == "HellDog")
      {
	    ...
	    isDodge = true;
	    //1秒后退出dodge状态
	    this->runAction(Sequence::create(DelayTime::create(1), CallFunc::create([=] {isDodge = false; }), nullptr));
