...
public:
	...
	virtual void update(float dt);//更新函数，每帧调用一次
	virtual void moveAnimation(Vector<SpriteFrame*> frame, int actionTag); // 人物移动动画
	virtual Sprite* getBody() { return m_body; }//获取身体
	virtual bool getKeyBoardState(EventKeyboard::KeyCode key) { return keyMap[key]; }
	virtual float getSpeed() { return speed; }//获取速度
	bool testCol(Sprite* other);//检查碰撞精灵
	bool testCol(TMXTiledMap* map, TMXLayer* layer);//检查碰撞图层的重载
...
private:
	...
	TMXTiledMap* map = TMXTiledMap::create("Map/map-un.tmx");//玩家所处的地图
	TMXLayer* layer = map->getLayer("wall");//地图碰撞层
